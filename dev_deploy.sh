...
wait_for_postgres() {
    local db_user=$(grep -E "^DB_USERNAME=" .env | cut -d= -f2 | tr -d '"' | tr -d "'")
    local db_name=$(grep -E "^DB_DATABASE=" .env | cut -d= -f2 | tr -d '"' | tr -d "'")
    db_user=${db_user:-lms_user}
    db_name=${db_name:-lms_local_db}

    log "Очікую готовність PostgreSQL (user=$db_user, db=$db_name)..."
    for i in {1..60}; do
        if docker compose exec -T pgsql pg_isready -U "$db_user" -d "$db_name" &>/dev/null; then
            # Реальний запит від нашого користувача, не від 'postgres' (його в PG-контейнері нема)
            if docker compose exec -T pgsql psql -U "$db_user" -d "$db_name" -c "SELECT 1" &>/dev/null; then
...
# Laravel вимагає writable bootstrap/cache і storage/
# В контейнері sail (uid 1337), на хості ваш uid — 777 найпростіше для локалки
log "Перевірка прав bootstrap/cache і storage..."
mkdir -p bootstrap/cache
chmod -R 777 bootstrap/cache storage 2>/dev/null || \
    docker run --rm -v "$(pwd):/app" -w /app ubuntu:24.04 chmod -R 777 bootstrap/cache storage
...
if [ "$MODE" = "fresh" ]; then
    ...
    log "Видаляю vendor / node_modules / кеш (через контейнер, щоб обійти права sail:1337)..."
    docker run --rm -v "$(pwd):/app" -w /app ubuntu:24.04 \
        rm -rf vendor node_modules bootstrap/cache public/build 2>/dev/null || true

    # bootstrap/cache потрібна Laravel зі змогою запису для sail (uid 1337) і хоста
    mkdir -p bootstrap/cache
